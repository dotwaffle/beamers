...
ALTER TABLE events DROP COLUMN public_slug;
...
CREATE TABLE upload_links (
	...
	created_at datetime NOT NULL,
	event_id integer NOT NULL,
	CONSTRAINT upload_links_events_upload_links
		FOREIGN KEY (event_id) REFERENCES events (id)
		ON UPDATE NO ACTION ON DELETE NO ACTION
);
CREATE UNIQUE INDEX upload_links_token_hash_key ON upload_links (token_hash);
CREATE INDEX uploadlink_event_id_target_type_target_id_created_at
	ON upload_links (event_id, target_type, target_id, created_at);
