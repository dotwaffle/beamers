...
func (_c *RecoveryCodeCreate) SetAccountID(v int) *RecoveryCodeCreate {
...
func (_c *RecoveryCodeCreate) SetAccount(v *Account) *RecoveryCodeCreate {
...
func (_c *RecoveryCodeCreate) defaults() error {
...
func (_c *RecoveryCodeCreate) check() error {
	if _, ok := _c.mutation.AccountID(); !ok {
		return &ValidationError{Name: "account_id", err: errors.New(`ent: missing required field "RecoveryCode.account_id"`)}
	}
	if _, ok := _c.mutation.CodeHash(); !ok {
		return &ValidationError{Name: "code_hash", err: errors.New(`ent: missing required field "RecoveryCode.code_hash"`)}
	}
	if v, ok := _c.mutation.CodeHash(); ok {
		if err := recoverycode.CodeHashValidator(v); err != nil {
			return &ValidationError{Name: "code_hash", err: fmt.Errorf(`ent: validator failed for field "RecoveryCode.code_hash": %w`, err)}
		}
	}
	if _, ok := _c.mutation.CreatedAt(); !ok {
	...
	if len(_c.mutation.AccountIDs()) == 0 {
		return &ValidationError{Name: "account", err: errors.New(`ent: missing required edge "RecoveryCode.account"`)}
	}
	return nil
}
