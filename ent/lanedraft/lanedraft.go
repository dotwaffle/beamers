// Code generated by ent, DO NOT EDIT.

package lanedraft

import (
	"entgo.io/ent"
	"entgo.io/ent/dialect/sql"
	"entgo.io/ent/dialect/sql/sqlgraph"
)

const (
	// Label holds the string label denoting the lanedraft type in the database.
	Label = "lane_draft"
	// FieldID holds the string denoting the id field in the database.
	FieldID = "id"
	// FieldLaneID holds the string denoting the lane_id field in the database.
	FieldLaneID = "lane_id"
	// FieldLocationID holds the string denoting the location_id field in the database.
	FieldLocationID = "location_id"
	// FieldName holds the string denoting the name field in the database.
	FieldName = "name"
	// FieldRetired holds the string denoting the retired field in the database.
	FieldRetired = "retired"
	// EdgeLane holds the string denoting the lane edge name in mutations.
	EdgeLane = "lane"
	// EdgeLocation holds the string denoting the location edge name in mutations.
	EdgeLocation = "location"
	// Table holds the table name of the lanedraft in the database.
	Table = "lane_drafts"
	// LaneTable is the table that holds the lane relation/edge.
	LaneTable = "lane_drafts"
	// LaneInverseTable is the table name for the Lane entity.
	// It exists in this package in order to avoid circular dependency with the "lane" package.
	LaneInverseTable = "lanes"
	// LaneColumn is the table column denoting the lane relation/edge.
	LaneColumn = "lane_id"
	// LocationTable is the table that holds the location relation/edge.
	LocationTable = "lane_drafts"
	// LocationInverseTable is the table name for the Location entity.
	// It exists in this package in order to avoid circular dependency with the "location" package.
	LocationInverseTable = "locations"
	// LocationColumn is the table column denoting the location relation/edge.
	LocationColumn = "location_id"
)

// Columns holds all SQL columns for lanedraft fields.
var Columns = []string{
	FieldID,
	FieldLaneID,
	FieldLocationID,
	FieldName,
	FieldRetired,
}

// ValidColumn reports if the column name is valid (part of the table columns).
func ValidColumn(column string) bool {
	for i := range Columns {
		if column == Columns[i] {
			return true
		}
	}
	return false
}

// Note that the variables below are initialized by the runtime
// package on the initialization of the application. Therefore,
// it should be imported in the main as follows:
//
//	import _ "github.com/dotwaffle/beamers/ent/runtime"
var (
	Hooks  [2]ent.Hook
	Policy ent.Policy
	// NameValidator is a validator for the "name" field. It is called by the builders before save.
	NameValidator func(string) error
	// DefaultRetired holds the default value on creation for the "retired" field.
	DefaultRetired bool
)

// OrderOption defines the ordering options for the LaneDraft queries.
type OrderOption func(*sql.Selector)

// ByID orders the results by the id field.
func ByID(opts ...sql.OrderTermOption) OrderOption {
	return sql.OrderByField(FieldID, opts...).ToFunc()
}

// ByLaneID orders the results by the lane_id field.
func ByLaneID(opts ...sql.OrderTermOption) OrderOption {
	return sql.OrderByField(FieldLaneID, opts...).ToFunc()
}

// ByLocationID orders the results by the location_id field.
func ByLocationID(opts ...sql.OrderTermOption) OrderOption {
	return sql.OrderByField(FieldLocationID, opts...).ToFunc()
}

// ByName orders the results by the name field.
func ByName(opts ...sql.OrderTermOption) OrderOption {
	return sql.OrderByField(FieldName, opts...).ToFunc()
}

// ByRetired orders the results by the retired field.
func ByRetired(opts ...sql.OrderTermOption) OrderOption {
	return sql.OrderByField(FieldRetired, opts...).ToFunc()
}

// ByLaneField orders the results by lane field.
func ByLaneField(field string, opts ...sql.OrderTermOption) OrderOption {
	return func(s *sql.Selector) {
		sqlgraph.OrderByNeighborTerms(s, newLaneStep(), sql.OrderByField(field, opts...))
	}
}

// ByLocationField orders the results by location field.
func ByLocationField(field string, opts ...sql.OrderTermOption) OrderOption {
	return func(s *sql.Selector) {
		sqlgraph.OrderByNeighborTerms(s, newLocationStep(), sql.OrderByField(field, opts...))
	}
}
func newLaneStep() *sqlgraph.Step {
	return sqlgraph.NewStep(
		sqlgraph.From(Table, FieldID),
		sqlgraph.To(LaneInverseTable, FieldID),
		sqlgraph.Edge(sqlgraph.O2O, true, LaneTable, LaneColumn),
	)
}
func newLocationStep() *sqlgraph.Step {
	return sqlgraph.NewStep(
		sqlgraph.From(Table, FieldID),
		sqlgraph.To(LocationInverseTable, FieldID),
		sqlgraph.Edge(sqlgraph.M2O, true, LocationTable, LocationColumn),
	)
}
