...
type AttachmentQuery struct {
	...
	withVersions *AttachmentVersionQuery
...
func (_q *AttachmentQuery) QueryVersions() *AttachmentVersionQuery {
	query := (&AttachmentVersionClient{config: _q.config}).Query()
...
func (_q *AttachmentQuery) WithVersions(opts ...func(*AttachmentVersionQuery)) *AttachmentQuery {
	query := (&AttachmentVersionClient{config: _q.config}).Query()
...
//		GroupBy(attachment.FieldEventID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Attachment.Query().
//		Select(attachment.FieldEventID).
//		Scan(ctx, &v)
...
func (_q *AttachmentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Attachment, error) {
	...
	if query := _q.withVersions; query != nil {
		if err := _q.loadVersions(ctx, query, nodes,
			func(n *Attachment) { n.Edges.Versions = []*AttachmentVersion{} },
			func(n *Attachment, e *AttachmentVersion) { n.Edges.Versions = append(n.Edges.Versions, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *AttachmentQuery) loadVersions(ctx context.Context, query *AttachmentVersionQuery, nodes []*Attachment, init func(*Attachment), assign func(*Attachment, *AttachmentVersion)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(attachmentversion.FieldAttachmentID)
	}
	query.Where(predicate.AttachmentVersion(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(attachment.VersionsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "attachment_id" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil
