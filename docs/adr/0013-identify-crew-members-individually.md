...
ADR 0054 extends Accounts to optional attendee participation and brings WebAuthn, federation, recovery, and Account-bound submissions into version one.
This decision's individual Crew identity and Event Grant model remain in force.
...
Displays authenticate through Enrollment credentials, while public Views remain open.
Password authentication remains supported, but password possession is not intrinsic to an Account.
An Account may have no password credential, one password credential, one or more WebAuthn credentials, or both kinds.
...
Passkeys and roaming security keys use WebAuthn and require a stable HTTPS origin and relying-party identity.
